    ...
    <div className="max-h-[32rem] overflow-y-auto space-y-3">
      ...
      {rows.map(({ album, request, key }) => <article key={`${album.id}:${key}`} className="rounded-lg border border-border p-4 space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <span className={`rounded-full px-2.5 py-1 text-xs ${request.status === "pending" ? "bg-amber-500/15 text-amber-500" : "bg-primary/10 text-primary"}`}>{request.status}</span>
        </div>
        ...
        <p className="text-xs text-muted-foreground">Requested {new Date(request.requestedAt).toLocaleString()}{request.approvedAt ? ` · Approved ${new Date(request.approvedAt).toLocaleString()}` : ""}</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {request.status === "pending" && <Button size="sm" disabled={busy !== null || !isServerMode() || !request.sessionKey || request.method !== "bank-transfer"} onClick={() => approve(album, request, key)}>{busy === `${album.id}:${key}` ? "Approving…" : "Confirm transfer & approve"}</Button>}
        </div>
