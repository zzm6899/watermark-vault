...
export default function GalleryPurchaseStatus({ requests, photos, paidCount, ready, refreshing, onRefresh, onBankDetails, onSelect }: {
  ...
  return <section aria-label="Purchase status" className="border-b border-border pb-4 space-y-3">
    ...
    {requests.map((request, index) => <div key={request.id || index} className="border border-border rounded p-3 text-sm space-y-2">
      <p className="font-medium">{request.status === "cancelled" ? "Download request cancelled" : request.status === "pending" ? "Awaiting bank transfer confirmation" : ready ? request.billablePhotoIds ? "Payment confirmed — paid photos unlocked" : "Payment confirmed — ready to download" : "Payment confirmed — downloads unavailable"}</p>
      ...
      {request.cancelledAt && <p className="text-xs text-muted-foreground">Cancelled {new Date(request.cancelledAt).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric" })}</p>}
      ...
      {["approved", "completed"].includes(request.status) && ready && onSelect && <Button variant="outline" size="sm" onClick={() => onSelect(request)}>Review these photos</Button>}
    </div>)}
  </section>;
}
