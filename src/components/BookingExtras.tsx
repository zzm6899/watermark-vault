...
export function BookingExtras({ event, quantities, onChange }: { event: EventType; quantities: Record<string, number>; onChange: (value: Record<string, number>) => void }) {
  ...
  return <fieldset className="rounded-xl border border-border bg-secondary/20 p-4 space-y-4">
    ...
    {event.extras.map(extra => <div key={extra.id} className="flex items-center justify-between gap-3">
      <label htmlFor={`extra-${extra.id}`} className="min-w-0 text-sm"><span className="block font-medium">{extra.name}</span><span className="text-muted-foreground">${extra.price.toFixed(2)} each · up to {extra.maxQuantity}</span></label>
      <Input id={`extra-${extra.id}`} type="number" inputMode="numeric" min={0} max={extra.maxQuantity} step={1} value={quantities[extra.id] || 0}
        onChange={e => onChange({ ...quantities, [extra.id]: Math.max(0, Math.min(extra.maxQuantity, Math.floor(Number(e.target.value) || 0))) })} className="w-24 shrink-0" />
    </div>)}
  </fieldset>;
}
