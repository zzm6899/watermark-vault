...
export default function PortfolioEditor() {
  ...
  const save = async () => { setSaving(true); try { await savePortfolioDraft(draft); const cleanDraft = { ...draft, webhookUrl: undefined, webhookUrlSet: draft.webhookUrl !== undefined ? !!draft.webhookUrl : draft.webhookUrlSet }; setDraft(cleanDraft); setSavedSnapshot(JSON.stringify(cleanDraft)); toast.success("Website draft saved"); } catch (error) { toast.error(error instanceof Error ? error.message : "Save failed"); } finally { setSaving(false); } };
  const publish = async () => { setSaving(true); try { await savePortfolioDraft(draft); const result = await publishPortfolio(); const cleanDraft = { ...draft, webhookUrl: undefined, webhookUrlSet: draft.webhookUrl !== undefined ? !!draft.webhookUrl : draft.webhookUrlSet }; setDraft(cleanDraft); setSavedSnapshot(JSON.stringify(cleanDraft)); setPublishedAt(result.publishedAt); toast.success("Website published"); } catch (error) { toast.error(error instanceof Error ? error.message : "Publish failed"); } finally { setSaving(false); } };
  const testWebhook = async () => { if (!draft.webhookUrl && !draft.webhookUrlSet) return toast.error("Enter a Discord webhook URL first"); setTestingWebhook(true); try { await testPortfolioWebhook(draft.webhookUrl || ""); toast.success("Test notification sent"); } catch (error) { toast.error(error instanceof Error ? error.message : "Webhook test failed"); } finally { setTestingWebhook(false); } };
