...
export function readProofingDraft(album: Album, sessionKey: string) {
  ...
  try {
    ...
    for (const requirement of album.proofingAddonRequirements || []) {
      ...
      if (Array.isArray(picks)) addonSelections[requirement.id] = [...new Set(picks.filter((id: unknown): id is string => typeof id === "string" && available.has(id) && (draft.photographerChooses === true || album.proofingPhotoSelection === "off" || (album.proofingPhotoSelection === "optional" && draft.photoIds.length === 0) || draft.photoIds.includes(id))))].slice(0, requirement.quantity) as string[];
    }
