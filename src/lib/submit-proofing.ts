...
type Submission = { photographerChooses?: boolean; addonPhotographerChoices?: string[]; addonSelections?: Record<string, string[]>; albumId: string; selectedPhotoIds: string[]; clientNote: string; submissionId: string; roundNumber: number; roundSentAt?: string };
  ...
  try {
    ...
    if (response.ok) {
      ...
      const round = album?.id === submission.albumId && album.proofingRounds?.find(item =>
        ...
        (item.photographerChooses === true) === (submission.photographerChooses === true) &&
        JSON.stringify([...(item.addonPhotographerChoices || [])].sort()) === JSON.stringify([...(submission.addonPhotographerChoices || [])].sort()) &&
