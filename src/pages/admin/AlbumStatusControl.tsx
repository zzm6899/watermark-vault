export default function AlbumStatusControl({ proofing, value, bookingStatus, disabled, onChange }: {
  ...
  return <div className="flex items-center gap-1.5 flex-wrap">
    <select aria-label="Gallery status" value={value} disabled={disabled} onClick={(event) => event.stopPropagation()} onChange={(event) => { event.stopPropagation(); onChange(event.target.value); }} className={`text-[10px] font-body px-2 py-0.5 rounded-full border-0 cursor-pointer appearance-none focus:outline-none focus:ring-1 focus:ring-primary/50 disabled:cursor-wait disabled:opacity-60 ${color}`}>
      {proofing ? <>
