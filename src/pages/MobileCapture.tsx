      ...
      >
        <div className="p-4">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isNextUp ? "bg-primary/20" : "bg-secondary"}`}>
              {isNextUp
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {isNextUp && <span className="w-2 h-2 rounded-full bg-primary animate-pulse flex-shrink-0" />}
              </div>
              <p className="text-xs font-body text-muted-foreground truncate">
                {bk.type} · {formatDate(bk.date)} {bk.time ? `· ${formatTime12(bk.time)}` : ""} {bk.duration ? `· ${formatDuration(bk.duration)}` : ""}
              </p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              {photoCount > 0 && (
                <span className="inline-flex items-center gap-1 text-xs font-body px-2 py-1 rounded-full bg-secondary border border-border text-muted-foreground">
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b border-border/50">
          <div className="p-4 space-y-3">
            {/* Top row */}
            <div className="flex items-center gap-3">
              <button onClick={() => navigate("/admin")} className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground">
                <ArrowLeft className="w-5 h-5" />
              </button>
              <h1 className="font-display text-xl text-foreground flex-1">Mobile Capture</h1>
              <div className="flex items-center gap-2">
                {isNative && (
                  <span className={`inline-flex items-center gap-1 text-xs font-body px-2.5 py-1 rounded-full border ${cameraConnected ? "border-primary/50 text-primary bg-primary/10" : "border-border text-muted-foreground bg-secondary"}`}>
                    <Usb className="w-3 h-3" />
                    {cameraConnected ? cameraName || "Z6III" : "No Camera"}
                  </span>
                )}
                <span className={`inline-flex items-center gap-1 text-xs font-body px-2.5 py-1 rounded-full border ${serverOnline ? "border-primary/50 text-primary bg-primary/10" : "border-destructive/50 text-destructive bg-destructive/10"}`}>
