...
function MobileCaptureInner() {
  ...
  // Listen for OS-level camera disconnect (unplug)
  useEffect(() => {
    if (!isNative) return;
    if (!CameraUsb.addListener) return;
    let handle: any = null;
    const setup = async () => {
      handle = await CameraUsb.addListener("cameraDisconnected" as any, () => {
        setCameraConnected(false);
        setCameraFiles([]);
        setCameraName("");
        if (watching) {
          setWatching(false);
          toast.info("Camera disconnected — live capture stopped");
        }
      });
    };
    setup();
  ...
  const importCameraFiles = async (handles: number[]) => {
    ...
    // Always re-check server — closure state can be stale, especially in live-capture callbacks
    ...
    try {
      ...
      console.log(`[import] ${imported.length} files from camera | server=${isOnline}`);
