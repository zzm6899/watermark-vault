...
function _mergePhotoArrays(existingPhotos, incomingPhotos) {
  return mergeAlbumPhotos(existingPhotos, _stripBakedFromPhotos(incomingPhotos) || []);
}
  ...
  if (idx < 0 && incoming.bookingId) {
    ...
    if (existingIdx >= 0) {
      ...
      const merged = { ...existing, ...incoming, id: existing.id,
        photos: mergeAlbumPhotos(existing.photos || [], incoming.photos || [], { replacePhotos, basePhotoIds }) };
...
app.post("/api/upload/speed-test", requireAdminOrScopedTenant, express.raw({ type: "application/octet-stream", limit: "8mb" }), express.text({ type: "text/plain", limit: "8mb" }), (req, res) => {
  const bytes = Buffer.isBuffer(req.body) ? req.body.length : typeof req.body === "string" ? Buffer.byteLength(req.body) : 0;
