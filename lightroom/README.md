...
4. After the client submits picks, choose **Sync client picks from folder**, then choose the shoot's top-level folder. The plug-in searches only catalogued RAW files in that folder and its subfolders, downloads selected proof JPEGs, creates a per-client collection, adds `Watermark Vault > Client > Selected`, and optionally rates them five stars.
